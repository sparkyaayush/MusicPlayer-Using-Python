...
class Player(tk.Frame):
	...
	def create_frames(self):
		...
		self.track.config(width=630,height=305)
		self.track.grid(row=0, column=0, padx=10)

		self.tracklist = tk.LabelFrame(self, text=f'PlayList - {str(len(self.playlist))}',
							font=("times new roman",15,"bold"),
							bg="black",fg="white",bd=5,relief=tk.GROOVE)
		self.tracklist.config(width=180,height=400)
		self.tracklist.grid(row=0, column=1, rowspan=3, pady=5)

		self.controls = tk.LabelFrame(self,
							font=("times new roman",16,"bold"),
							bg="black",fg="white",bd=2,relief=tk.GROOVE)
		self.controls.config(width=630,height=80)
		self.controls.grid(row=2, column=0, pady=5, padx=10)

	def track_widgets(self):
		self.canvas = tk.Label(self.track, image=img)
		self.canvas.configure(width=580, height=260)
		self.canvas.grid(row=0,column=0)

		self.songtrack = tk.Label(self.track, font=("times new roman",16),
						bg="black",fg="red")
		self.songtrack['text'] = 'BEATS MP3 Player'
	...
	def control_widgets(self):
		...
		self.loadSongs['text'] = 'Load Music'
...
if __name__=='__main__':
    ...
    root.wm_title('BEATS MP3 Player')
    ...
    shuffle=PhotoImage(file='images/shuffle.gif')
